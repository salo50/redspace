...
#if defined(NDEBUG)
#error "redspace cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uRSC) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zRSC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zRSC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zRSC spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zRSC without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zRSC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zRSC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zrsc");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zRSC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 55000 && nHeight >= 30001) {
        nSubsidy = 3 * COIN;
    } else if (nHeight <= 80000 && nHeight >= 55001) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 115000 && nHeight >= 80001) {
        nSubsidy = 12 * COIN;
    } else if (nHeight <= 135000 && nHeight >= 115001) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 150000 && nHeight >= 135001) {
        nSubsidy = 17 * COIN;
    } else if (nHeight <= 200000 && nHeight >= 150001) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 215000 && nHeight >= 200001) {
        nSubsidy = 55 * COIN;
    } else if (nHeight <= 265000 && nHeight >= 215001) {
        nSubsidy = 28 * COIN;
    } else if (nHeight <= 320000 && nHeight >= 265001) {
        nSubsidy = 32 * COIN;
    } else if (nHeight <= 400000 && nHeight >= 320001) {
        nSubsidy = 38 * COIN;
    } else if (nHeight <= 500000 && nHeight >= 400001) {
        nSubsidy = 44 * COIN;
    } else if (nHeight <= 612000 && nHeight >= 500001) {
        nSubsidy = 35 * COIN;
    } else if (nHeight <= 770000 && nHeight >= 612001) {
        nSubsidy = 48 * COIN;
    } else if (nHeight <= 890000 && nHeight >= 770001) {
        nSubsidy = 55 * COIN;
    } else if (nHeight <= 1024000 && nHeight >= 890001) {
        nSubsidy = 46 * COIN;
    } else if (nHeight <= 1100000 && nHeight >= 1024001) {
        nSubsidy = 60 * COIN;
    } else if (nHeight <= 1900000 && nHeight >= 1100001) {
        nSubsidy = 44 * COIN;
    } else if (nHeight <= 2400000 && nHeight >= 1900001) {
        nSubsidy = 38 * COIN;
    } else if (nHeight <= 3000000 && nHeight >= 2400001) {
        nSubsidy = 48 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZFUNCStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from redspace
...
{
    RenameThread("redspace-scriptch");
...
void RecalculateZRSCMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZRSC..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZRSC..."), percent);
        }
...
void RecalculateZRSCSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZRSC..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZRSC..."), percent);
        }
        ...
        //Remove spends from zRSC supply
...
bool RecalculateRSCSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating RSC supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating RSC supply..."), percent);
        }
...
bool UpdateZRSCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zRSC supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateRSCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zRSC money supply in the block index
    if (!UpdateZRSCSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zRSC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zRSCSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert RSC to zRSC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zRSC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zRSC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zRSC stake block is less than allowed start height", __func__);

    if (CZRSCStake* zRSC = dynamic_cast<CZRSCStake*>(stake)) {
        CBlockIndex* pindexFrom = zRSC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zRSC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zRSC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zRSC->GetValue()));
        if (nChecksum200 != zRSC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zRSC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZRSC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zRSC fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> RSCInputs;
        std::vector<CTxIn> zRSCInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zRSCInputs.push_back(stakeIn);
            }else{
                RSCInputs.push_back(stakeIn);
            }
        }
        const bool hasRSCInputs = !RSCInputs.empty();
        const bool hasZRSCInputs = !zRSCInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasRSCInputs)
                    ...
                    for (const CTxIn& RSCIn : RSCInputs){
                        if(RSCIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : RSCInputs) {
            ...
            // Now that this loop if completed. Check if we have zRSC inputs.
            if(hasZRSCInputs){
                for (const CTxIn& zRSCInput : zRSCInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zRSCInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zrsc");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zRSC mints and %d zRSC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // redspace: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zrsc", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zrsc", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
