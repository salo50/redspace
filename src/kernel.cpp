...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZRSCStake(spend));
    } else {
        ...
        CRSCStake* RSCInput = new CRSCStake();
        RSCInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(RSCInput);
    }
