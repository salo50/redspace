...
#include "ui_zrsccontroldialog.h"
...
std::set<std::string> ZRSCControlDialog::setSelectedMints;
std::set<CMintMeta> ZRSCControlDialog::setMints;

bool CZRSCControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRSCControlDialog::COLUMN_DENOMINATION || column == ZRSCControlDialog::COLUMN_VERSION || column == ZRSCControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRSCControlDialog::ZRSCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRSCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRSCControlDialog::~ZRSCControlDialog()
{
    delete ui;
}

void ZRSCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRSCControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRSCControlWidgetItem* itemDenom(new CZRSCControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRSCControlWidgetItem *itemMint = new CZRSCControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zrscTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zrscTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRSC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRSC seed used to mint this zRSC is not the same as currently hold in the wallet";
            else
...
void ZRSCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRSCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRSC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRSCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRSCControlDialog::GetSelectedMints()
{
...
void ZRSCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
