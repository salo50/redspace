...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RSC);
    ...
    if (!settings.contains("nAnonymizeRedSpaceAmount"))
        settings.setValue("nAnonymizeRedSpaceAmount", 1000);

    nAnonymizeRedSpaceAmount = settings.value("nAnonymizeRedSpaceAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRedSpaceAmount"))
        SoftSetArg("-anonymizeredspaceamount", settings.value("nAnonymizeRedSpaceAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in redspace.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRedSpaceAmount:
            return QVariant(nAnonymizeRedSpaceAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeRedSpaceAmount:
            nAnonymizeRedSpaceAmount = value.toInt();
            settings.setValue("nAnonymizeRedSpaceAmount", nAnonymizeRedSpaceAmount);
            emit anonymizeRedSpaceAmountChanged(nAnonymizeRedSpaceAmount);
            break;
        case CoinControlFeatures:
