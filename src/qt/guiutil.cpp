...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a redspace address (e.g. %1)").arg("RtbN8pLLowtAbKvGH7xHg56hkMmDNEP6JC"));
...
{
    // return if URI is not valid or is no redspace: URI
...
{
    // Convert redspace:// to redspace:
    //
    //    Cannot handle this later, because redspace:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::RSC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open redspace.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "redspace.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for redspace.lnk
...
{
    return GetAutostartDir() / "redspace.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a redspace.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=redspace\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the redspace app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add redspace app to startup item list
