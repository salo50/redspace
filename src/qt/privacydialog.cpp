                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zRSC ought to be enough for anybody." - Bill Gates, 2017
    ui->zRSCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRSCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRSCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzRSC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRSC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zRSC, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRSC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRSC in ") +
...
void PrivacyDialog::on_pushButtonSpendzRSC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRSC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zRSC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zRSC
        sendzRSC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRSC
    sendzRSC();
}

void PrivacyDialog::on_pushButtonZRSCControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZRSCControlDialog* ZRSCControl = new ZRSCControlDialog(this);
    ZRSCControl->setModel(walletModel);
    ZRSCControl->exec();
}

void PrivacyDialog::setZRSCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelZRSCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRSC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid RedSpace Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zRSCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRSC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " RSC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRSC</b>";
    ...
    // use mints from zRSC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRSCControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZRSCControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zRSC was spent successfully update the addressbook with the label
    ...
    // Clear zrsc selector in case it was used
    ZRSCControlDialog::setSelectedMints.clear();
    ui->labelZRSCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRSC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " rsc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zRSC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zrscTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRSC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRSC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRSC "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zRSC "));
    ui->labelzRSCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zRSC </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zRSC </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zRSC </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzRSCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzRSC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zRSC
        ui->pushButtonMintzRSC->setEnabled(false);
        ui->pushButtonMintzRSC->setToolTip(tr("zRSC is currently disabled due to maintenance."));

        // Spend zRSC
        ui->pushButtonSpendzRSC->setEnabled(false);
        ui->pushButtonSpendzRSC->setToolTip(tr("zRSC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zRSC
        ui->pushButtonMintzRSC->setEnabled(true);
        ui->pushButtonMintzRSC->setToolTip(tr("PrivacyDialog", "Enter an amount of RSC to convert to zRSC", 0));

        // Spend zRSC
        ui->pushButtonSpendzRSC->setEnabled(true);
        ui->pushButtonSpendzRSC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
