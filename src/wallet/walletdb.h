...
public:
    ...
    bool WriteZRSCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRSCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRSCSeed_deprecated(uint256& seed);
    bool EraseZRSCSeed();
    bool EraseZRSCSeed_deprecated();

    bool WriteZRSCCount(const uint32_t& nCount);
    bool ReadZRSCCount(uint32_t& nCount);
