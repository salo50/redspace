...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zrscspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 20000 RSC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zRSC send
enum ZerocoinSpendStatus {
    ZRSC_SPEND_OKAY = 0,                            // No error
    ZRSC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZRSC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZRSC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZRSC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZRSC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZRSC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZRSC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZRSC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZRSC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZRSC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZRSC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZRSC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZRSC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZRSC_SPENT_USED_ZRSC = 14,                      // Coin has already been spend
    ZRSC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZRSC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZRSCBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZRSCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzrscAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzRSCTracker> zrscTracker;
    ...
    void setZWallet(CzRSCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zrscTracker = std::unique_ptr<CzRSCTracker>(new CzRSCTracker(strWalletFile));
    }

    CzRSCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZRSCAutoBackups(bool fEnabled)
    {
    ...
    /** zRSC reset */
    boost::signals2::signal<void()> NotifyzRSCReset;
