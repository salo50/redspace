...
class DBConstructor: public Constructor {
 ...
 private:
  void NewDB() {
    ...
    options.write_buffer_size = 20000;  // Something small to force merging
...
TEST(TableTest, ApproximateOffsetOfPlain) {
  ...
  ASSERT_TRUE(Between(c.ApproximateOffsetOf("k04"),   20000,  11000));
...
TEST(TableTest, ApproximateOffsetOfCompressed) {
  ...
  std::string tmp;
  c.Add("k01", "hello");
  c.Add("k02", test::CompressibleString(&rnd, 0.25, 20000, &tmp));
  c.Add("k03", "hello3");
  c.Add("k04", test::CompressibleString(&rnd, 0.25, 20000, &tmp));
  std::vector<std::string> keys;
  KVMap kvmap;
  Options options;
  options.block_size = 1024;
  ...
  const int expected = 2500;  // 20000 * compression ratio (0.25)
