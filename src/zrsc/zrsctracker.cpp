...
CzRSCTracker::CzRSCTracker(std::string strWalletFile)
{
...
CzRSCTracker::~CzRSCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRSCTracker::Init()
{
...
bool CzRSCTracker::Archive(CMintMeta& meta)
{
...
bool CzRSCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRSCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRSCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRSCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzRSCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzRSCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzRSCTracker::GetSerialHashes()
{
...
CAmount CzRSCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRSCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRSCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRSCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRSCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRSCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRSCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRSCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRSCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRSCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRSCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRSCWallet* zRSCWallet)
{
    bool iszRSCWalletInitialized = (NULL != zRSCWallet);
    CMintMeta meta;
    ...
    if (! iszRSCWalletInitialized)
        zRSCWallet = new CzRSCWallet(strWalletFile);
    meta.isSeedCorrect = zRSCWallet->CheckSeed(dMint);
    if (! iszRSCWalletInitialized)
        delete zRSCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRSCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRSCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRSCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRSCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRSCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRSCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRSCWallet* zRSCWallet = new CzRSCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zRSCWallet);
        }
        delete zRSCWallet;
        LogPrint("zero", "%s: added %d dzrsc from DB\n", __func__, listDeterministicDB.size());
    }
