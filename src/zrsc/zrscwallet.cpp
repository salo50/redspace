...
CzRSCWallet::CzRSCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrsc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRSCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRSCSeed_deprecated()) {
                    LogPrintf("%s: Updated zRSC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrsc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrsc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRSCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRSCCount(nCountLastUsed);
    else if (!walletdb.ReadZRSCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRSCWallet::Lock()
{
    seedMaster = 0;
}

void CzRSCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRSCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRSC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRSCWallet::LoadMintPoolFromDB()
{
...
void CzRSCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRSCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRSCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrscTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRSCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRSC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrscTracker which also adds to database
    pwalletMain->zrscTracker->Add(dMint, true);
...
void CzRSCWallet::SeedToZRSC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRSCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRSCWallet::GenerateDeterministicZRSC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRSCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRSCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRSCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
