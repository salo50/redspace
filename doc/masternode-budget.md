...
redspace now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<redspace_address\> \<monthly_payment_redspace\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ redspace, _12_ times over the course of a year totaling _24000_ redspace.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<redspace_address\> \<monthly_payment_redspace\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` redspace.
 ...
 * vote-many          - Vote on a redspace initiative
 * vote-alias         - Vote on a redspace initiative
 * vote               - Vote on a redspace initiative/budget
